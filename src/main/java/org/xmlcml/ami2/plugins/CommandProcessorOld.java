...
public class CommandProcessorOld {
	...
	private static final Logger LOG = Logger.getLogger(CommandProcessorOld.class);
	static {
	...
	private CommandProcessorOld() {
		
	}

	public CommandProcessorOld(File projectDir) {
...
//			pluginOption.setProject(projectDir);
			System.out.println("filterzz: "+pluginOption+" | "+pluginOption.getSnippetsName());
			pluginOption.runFilterResultsXMLOptions();
			System.out.println("summary: "+pluginOption.getProjectDir());
	...
	public static void main(String[] args) throws IOException {
		CommandProcessorOld commandProcessor = new CommandProcessorOld();
		if (args.length == 0) {
			help();
		} else {
			commandProcessor.runDefaults(args);
		}
	}
	...
	public void runDefaults(String[] args) throws IOException {
		...
		setProjectDir(projectDir);
		List<String> commands = CommandProcessorOld.getDefaultCommands();
		if (args.length > 1) {
